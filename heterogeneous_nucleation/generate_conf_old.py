...
def _5_insert_PW():
    ...
    SOL_filtered = u_ice.select_atoms("resname SOL and name OW and (prop x > 10) and (prop x < 50) and (prop y > 10) and (prop y < 50)").resids
    SOL_origin_id = {str(x) for x in SOL_filtered}
    SOL_origin = u_ice.select_atoms(f"resid {' '.join(SOL_origin_id)}")
